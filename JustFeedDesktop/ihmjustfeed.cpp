...
IHMJustFeed::IHMJustFeed(QWidget *parent)
    : QWidget(parent), configurationDistributeur(nullptr),
...
/**
 * @brief méthode qui retourne le distributeur dont le nom est passé en
 * paramètre
 */
 ...
 */
Produit *IHMJustFeed::getProduit(int index) const { return produits[index]; }
 ...
 */
int IHMJustFeed::getNbProduits() const { return produits.size(); }
...
void IHMJustFeed::configurerDistributeur() {
  if (configurationDistributeur == nullptr) {
    if (numeroDistributeurSelectionne != -1)
      configurationDistributeur = new ConfigurationDistributeur(
          distributeurs[numeroDistributeurSelectionne], this);
    else
      return;
  } else
    return;
...
void IHMJustFeed::selectionnerDistributeur(int ligne, int colonne) {
  ...
  nomDistributeur =
      tableWidgetDistributeurs->item(ligne, COLONNE_DISTRIBUTEUR_NOM);
...
void IHMJustFeed::selectionnerDistributeur(QTableWidgetItem *item) {
  QTableWidgetItem *nomDistributeur;
  nomDistributeur =
      tableWidgetDistributeurs->item(item->row(), COLONNE_DISTRIBUTEUR_NOM);
  qDebug() << Q_FUNC_INFO << "distributeur"
           << getDistributeur(nomDistributeur->data(0).toString())->getNom();
  afficherDistributeur(getDistributeur(nomDistributeur->data(0).toString()));
}
...
void IHMJustFeed::initialiserEvenements() {
  ...
  connect(tableWidgetDistributeurs, SIGNAL(itemPressed(QTableWidgetItem *)),
          this, SLOT(selectionnerDistributeur(QTableWidgetItem *)));
  connect(tableWidgetDistributeurs, SIGNAL(itemClicked(QTableWidgetItem *)),
          this, SLOT(selectionnerDistributeurAIntervenir(QTableWidgetItem *)));
  connect(listeDistributeurs, SIGNAL(currentIndexChanged(int)), this,
          SLOT(selectionnerDistributeur(int)));
  connect(boutonConfigurer, SIGNAL(clicked()), this,
          SLOT(configurerDistributeur()));
  connect(boutonPlanifier, SIGNAL(clicked()), this,
          SLOT(planifierIntervention()));
  connect(boutonValider, SIGNAL(clicked()), this,
          SLOT(afficherFenetreAccueil()));
}
...
void IHMJustFeed::initialiserDistributeurs() {
  distributeurs.push_back(
      new Distributeur("distributeur-1-sim", "Grand Frais",
                       ...
                       "Orange", "Distributeur de fruits secs",
                       QDate::fromString("2022-01-08", "yyyy-MM-dd"),
                       {"44.11161", "4.84856", "0"}));
  distributeurs.push_back(new Distributeur(
      "distributeur-2-sim", "Carrefour", "390 Rue Jean Marie Tjibaou", "84000",
      "Avignon", "Distributeur de fruits secs",
      QDate::fromString("2022-03-09", "yyyy-MM-dd"),
      {"43.92844", "4.79247", "0"}));
  distributeurs.push_back(new Distributeur(
      "distributeur-3", "Cosy Primeurs", "292 Route de Boulbon", "13570",
      "Barbentane", "Distributeur de fruits secs",
      QDate::fromString("2022-01-10", "yyyy-MM-dd"),
      {"43.90252", "4.75280", "0"}));

  Produit *pruneaux = new Produit(
      "Pruneaux", "Maître Prunille",
      "Les Pruneaux d'Agen dénoyautés Maître Prunille sont une "
      "délicieuse friandise à déguster à tout moment de la journée.",
      "761234567890", 1.15);
  Produit *abricot =
      new Produit("Abricots secs", "Maître Prunille",
                  "L'abricot moelleux, une gourmandise tendre et fruitée !",
                  "761234566000", 1.13);
  Produit *cranberries =
      new Produit("Cranberries", "SEEBERGER",
                  "Cranberries tranchées sucrées séchées", "761234569000", 2.1);
  Produit *banane = new Produit("Banane CHIPS", " BIO VILLAGE",
                                "Banane CHIPS bio ", "761234560008", 0.76);
  Produit *raisin = new Produit(
      "Raisin sec", "Petit Prix",
      "Raisins secs, huile végétale (graine de coton)", "761264569090", 0.39);
  Produit *fruitsSec = new Produit("fruits sec", "FRUIDYLLIC",
                                   "Peut se manger tel que sans préparation.",
                                   "761234960940", 1.06);
  Produit *cacahuete = new Produit(
      "Cacahuète", "Carrefour",
  ...
  distributeurs[0]->ajouterBac(Bac(pruneaux, 50, 50, 25));
  distributeurs[0]->ajouterBac(Bac(abricot, 0, 100, 25));
  distributeurs[0]->ajouterBac(Bac(cranberries, 15, 30, 25));
  qDebug() << Q_FUNC_INFO << "Distributeur" << distributeurs[0]->getNom()
           << "NbBacs" << distributeurs[0]->getNbBacs();
  distributeurs[1]->ajouterBac(Bac(banane, 0, 12, 18));
  distributeurs[1]->ajouterBac(Bac(raisin, 0, 12, 18));
  distributeurs[1]->ajouterBac(Bac(fruitsSec, 0, 12, 18));
  qDebug() << Q_FUNC_INFO << "Distributeur" << distributeurs[1]->getNom()
           << "NbBacs" << distributeurs[1]->getNbBacs();
  distributeurs[2]->ajouterBac(Bac(cacahuete, 9, 12, 21));
  distributeurs[2]->ajouterBac(Bac(soja, 6, 12, 21));
  distributeurs[2]->ajouterBac(Bac(basilic, 10.8, 12, 21));
  qDebug() << Q_FUNC_INFO << "Distributeur" << distributeurs[2]->getNom()
           << "NbBacs" << distributeurs[2]->getNbBacs();
...
void IHMJustFeed::afficherDistributeurTable(const Distributeur &distributeur) {
  ...
  tableWidgetDistributeurs->setItem(nb - 1, COLONNE_DISTRIBUTEUR_NOM,
                                    itemEnseigne);
  tableWidgetDistributeurs->setItem(nb - 1, COLONNE_DISTRIBUTEUR_ADRESSE,
                                    itemAdresse);
  tableWidgetDistributeurs->setItem(nb - 1, COLONNE_DISTRIBUTEUR_VILLE,
                                    itemVille);
  tableWidgetDistributeurs->setItem(nb - 1, COLONNE_DISTRIBUTEUR_CODEPOSTAL,
                                    itemCodePostal);
  tableWidgetDistributeurs->setItem(nb - 1, COLONNE_DISTRIBUTEUR_INTERVENTION,
                                    itemIntervention);
