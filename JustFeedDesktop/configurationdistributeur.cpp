...
ConfigurationDistributeur::ConfigurationDistributeur(Distributeur *distributeur,
                                                     IHMJustFeed *parent)
    : QDialog(parent), distributeur(distributeur), ihmJustFeed(parent) {
  qDebug() << Q_FUNC_INFO;
  qDebug() << Q_FUNC_INFO << "deviceID" << distributeur->getDeviceID() << "nom"
           << distributeur->getNom() << "description"
           << distributeur->getDescription() << "adresse"
           << "codePostal" << distributeur->getCodePostal() << "ville"
           << distributeur->getVille() << "latitude"
...
void ConfigurationDistributeur::selectionnerNouveauProduit(int numeroBac) {
  labelsPrix[numeroBac]->setText(
      "Prix : " +
      QString::number(ihmJustFeed->getPrixProduit(
          choixNouveauProduit[numeroBac]->currentIndex())) +
      " €");
}
...
void ConfigurationDistributeur::changerLePrix(const int numeroBac) {
  QString nouveauPrix =
      QString::number(editionsNouveauPrix[numeroBac]->value());
  labelsPrix[numeroBac]->setText("Prix : " + nouveauPrix + " €");
  distributeur->setPrixProduit(numeroBac,
                               editionsNouveauPrix[numeroBac]->value());
...
void ConfigurationDistributeur::changerLeProduit(const int numeroBac) {
  ...
  QString nouveauPrix = QString::number(ihmJustFeed->getPrixProduit(
      choixNouveauProduit[numeroBac]->currentIndex()));
  labelsPrix[numeroBac]->setText("Prix : " + nouveauPrix + " €");
}
...
void ConfigurationDistributeur::initialiserWidgets() {
  for (int i = 0; i < distributeur->getNbBacs(); i++) {
    ...
    labelsPrix[i]->setText(
        "Prix : " + QString::number(distributeur->getProduitPrix(i)) + " €");
...
void ConfigurationDistributeur::initialiserEvenements() {
  for (int i = 0; i < distributeur->getNbBacs(); i++) {
    connect(choixNouveauProduit[i],
            ...
            [this, i]() { selectionnerNouveauProduit(i); });
    connect(boutonsChangerPrix[i], &QPushButton::clicked, this,
            [this, i]() { changerLePrix(i); });
    connect(boutonsChangerProduit[i], &QPushButton::clicked, this,
            [this, i]() { changerLeProduit(i); });
    connect(boutonSuppressionBac[i], &QPushButton::clicked, this,
            [this, i]() { supprimerBac(i); });
  }
...
void ConfigurationDistributeur::initialiserNouveauBac(int numeroBac) {
  ...
  labelsProduit[numeroBac]->setText(" : " +
                                    distributeur->getNomProduitBac(numeroBac));
